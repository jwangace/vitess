...
func registerTabletEnvFlags(fs *pflag.FlagSet) {
	...
	fs.Int64Var(&currentConfig.ConsolidatorQueryWaiterCap, "consolidator-query-waiter-cap", defaultConfig.ConsolidatorQueryWaiterCap, "Configure the maximum number of clients we allow to wait on the consolidator for each query.")
